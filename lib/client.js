...
const hasClientCertificate = (ktn) => {
  return ktn.pfxFilepath && ktn.pfxPassword
}

const createUrl = (ktn) => {
  let domain = ktn.domain

  if (hasClientCertificate(ktn)) {
    // クライアント証明書を使ったアクセスの場合 s.cybozu.com ドメインになるため、s. がなければ付与する
    domain = domain.replace(/(\.s)?(\.cybozu\.com)$/, '.s$2')
  }

  const basePath = ktn.guestSpaceId ? `k/guest/${ktn.guestSpaceId}/v1` : 'k/v1'
  return `https://${domain}/${basePath}/${ktn.command}`
}
